...
def retrieve(query: str, limit: int = 5) -> list[dict[str, Any]]:
    ...
    The index is small and may contain raw keys. Raw keys improve recall but
    are never returned as memory content. Full entry files are opened only
    after ranking the index candidates.
    """
    ...
    # Keep weights synchronized with the published specification when possible.
    _ = retrieval

    q = normalize(query)
    ranked: list[tuple[float, dict[str, Any]]] = []
    for entry in index.get("entries", []):
        semantic = field_score(q, entry.get("semantic_keys", []))
        raw = field_score(q, entry.get("raw_keys", []))
        alias = field_score(q, entry.get("aliases", []))
        entity = field_score(q, entry.get("entities", []))
        exact = max((phrase_similarity(q, x) for x in entry.get("semantic_keys", []) + entry.get("raw_keys", []) + entry.get("aliases", [])), default=0.0)
        specificity = min(1.0, len(tokens(q)) / 8.0)
        score = (
    ...
    for score, entry in ranked:
        if score < threshold or len(results) >= min(limit, 8):
            break
        ref = entry.get("content_ref")
        payload: dict[str, Any] = {"id": entry["id"], "score": round(score, 4), "index": entry}
        if ref:
            path = ROOT.parent / ref.removeprefix("memory/")
            if path.exists():
                payload["memory"] = load_json(path)
